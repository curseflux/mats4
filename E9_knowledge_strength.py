...
Under a user instruction to ignore the paragraph, how much of a false claim
still reaches the answer depends -- inversely -- on how explicitly the claim is
marked as belonging to the document:

    Gemma, atomic numbers    bare 62.1%   assert 19.8%   explicit 0.0%
    Qwen,  atomic numbers    bare 38.5%   assert 12.8%   explicit 0.0%

The awkward part is that this is only VISIBLE on atomic numbers. For element
symbols and capitals every cell sits at 0.0%: nothing leaks at all, so there is
no gradient to see. Two readings are currently indistinguishable:

    (a) the gradient is about atomic numbers -- a quirk of numeric answers;
    (b) the gradient is about WEAKLY-HELD facts, and atomic numbers are simply
        the relation this model holds loosely enough for framing to matter.

Every fact already carries the measurement that separates them. The screening
prompt asks the question with no paragraph at all, so

    knowledge strength = log P(true answer) - log P(false answer)

is a per-fact measure of how firmly the model holds it, uncontaminated by any
of the manipulations. It is recorded in the `screen` rows of the E8/injection
results, so this script needs no GPU.

    E8's median strength (Gemma):  atomic 15.3, symbols 34.9, capitals 37.9

Atomic numbers are held about half as firmly, which is (b)'s prediction for why
the gradient showed up there and nowhere else.

Four analyses, cheapest first:

  T1  Strength distributions, and whether the relations OVERLAP at all. If they
      do not, a matched-strength comparison is impossible in this dataset and
      strength stays confounded with relation -- which the script says outright
      rather than reporting a comparison that cannot be made.
  T2  The requested split: within each relation, divide facts at that
      relation's own median and re-run bare / assert / explicit. If the
      gradient appears in the weak half of symbols and capitals too, (b) wins.
  T3  The continuous version, which does not depend on any binning: per fact,
      leakage against strength, within each relation and pooled. If leakage
      falls with strength inside EVERY relation, that is consistent evidence
      even where the ranges do not overlap.
  T4  The decisive test, run only where T1 finds overlap: at MATCHED strength,
      do different relations leak the same amount? If yes, relation identity is
      irrelevant and strength is the variable.

A confound check runs throughout: weak and strong facts might simply differ in
baseline deference, so the neutral-policy rates are reported beside the
parametric ones.

Usage
-----
python E9_knowledge_strength.py \\
    --results gemma=results/gemma4_12b_conflict/analysis/injection/conventionality_results.jsonl \\
              qwen=results/qwen36_27b_conflict/analysis/injection/conventionality_results.jsonl \\
