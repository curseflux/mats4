...
Why this exists
---------------
Every result that compares `element_atomic_number` against the other two
relations is currently confounded.  `E8_conventionality.py`'s `rotation` mode
pairs subject *i* with row *i+1*'s answer, and `ELEMENT_ROWS` is in
atomic-number order, so for atomic numbers the false answer is *always* the true
answer plus one -- 348/348 rows in the committed Gemma run, 351/351 in Qwen.
"The atomic number for Argon is 19" is the single most swallowable false claim
in the whole dataset; "The chemical symbol for Americium is Ne" is not.

Three findings currently rest on that comparison:

  * atomic numbers defer far more than symbols under a bare assertion
    (69.0% vs 2.5% in Gemma, neutral policy);
  * the ignore-the-paragraph leakage gradient (bare > assert > explicit) is
    visible *only* on atomic numbers;
  * the act effect (stipulate - assert) FLIPS SIGN on atomic numbers,
    -10.83 against +11.97 for symbols, which is the falsification that killed
    the stipulability account.

All three have a simpler candidate explanation that has never been tested:
the false answer is off by one.  `E9_knowledge_strength.py` does not rule it
out, because it defines strength as log P(true) - log P(that fact's assigned
false answer), so for atomic numbers "strength" is log P(18) - log P(19) -- a
hard discrimination for reasons that have nothing to do with how firmly the
fact is held.  The control inherits the confound it is meant to remove.

The design
----------
Hold the entity fixed and vary only how far the false answer is from the true
one.  For atomic numbers that distance is a real number line:

    Iron -> 27   (d=1)      Iron -> 28  (d=2)      Iron -> 31  (d=5)
    Iron -> 46   (d=20)     Iron -> 77  (random derangement)

The other two relations get a two-level version of the same manipulation --
the neighbouring element's symbol, and the neighbouring row's capital
(`CAPITAL_ROWS` is grouped by region, so that is a same-region capital) --
against a seeded random derangement.  This is exactly the difference between
the Gemma and Qwen E8 runs, so it also measures the size of that mismatch.

Outcomes, in order of how much they would change the write-up:

  1. The act effect is negative on atomic numbers at *every* distance.
     -> the sign flip is real, the stipulability account stays dead, and this
        script has bought a much stronger version of the same claim.
  2. The act effect is negative at d=1 and climbs toward zero or positive as
     d grows.  -> the falsification was a plausibility artefact, and the
        headline of section 5 has to be retracted.  Better to find this here.
  3. Deference tracks distance steeply in all three relations.  -> the
     interesting result is not about relations at all: it is that a model's
     willingness to adopt a false claim is governed by how near-miss the claim
     is, and "relation type" was a proxy for that all along.

Screening
---------
A fact is kept if, with no paragraph present, the model generates the true
answer AND prefers it to the *random-distance* false answer.  That criterion is
deliberately distance-independent: making survival depend on the manipulation
would let the manipulation choose the sample, and the paired contrasts need the
same facts at every distance.  Per-distance strength, log P(true) - log P(false
at distance d), is then recorded as a covariate on every row -- which is the
version of E9's control that does not inherit the confound.

Usage
-----
python E10_answer_plausibility.py --config config.yaml \\
    --out results/gemma4_12b_conflict/analysis/plausibility --validate-only

python E10_answer_plausibility.py --config config.yaml \\
    --out results/gemma4_12b_conflict/analysis/plausibility
"""
