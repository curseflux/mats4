...
Three semantically equivalent renderings of the same false paragraph move
Gemma's context-following from 0.8% to 72% on identical items with identical
false answers, and move the answer margin by ~31 logits -- more than the
explicit instruction "Based on this paragraph, answer the question below"
(~28 logits).  But the three bundles change FOUR things at once:

    claim       how the false binding is stated
                  "A chemistry handbook lists Ag, the chemical symbol for Iron"
                  "A textbook section about Iron uses Ag as the element's
                   chemical symbol throughout the chapter"
    filler      two sentences of topical padding
    question    "What is the chemical symbol for Iron?" / "Which chemical
                symbol represents Iron?" / "Give the chemical symbol of Iron."
    constraint  "Answer in one word." / "Respond with one word only." /
                "Output a single word and nothing else."

So "phrasing matters" is currently the only claim the data supports.  This
script decomposes the swing into those four factors.  The outcome decides what
kind of finding this is:

  * localises to `claim`      -> a semantic result. The two extreme claim
    wordings differ in whether the false binding reads as a STIPULATION
    ("this source uses Ag as the symbol") or an ASSERTION ("Ag is the symbol"),
    and the model treats stipulations as adoptable and assertions as errors.
  * localises to `constraint` -> output-format pressure, not source trust, is
    doing the work. Funnier, and arguably more worrying for eval design.
  * spread across all four    -> genuine diffuse brittleness. Weaker, but still
    an honest and reportable answer.

Design
------
`swap` (default, 10 cells/fact).  Per relation, take the highest- and
lowest-deference bundles, A and B.  Then:

    A                     both endpoints, to reproduce the known result
    B
    B + one factor from A  x4   SUFFICIENCY: does this factor alone move it?
    A - one factor to B    x4   NECESSITY:   does removing it collapse the effect?

Single swaps plus their complements also give a free additivity test: if the
four sufficiency effects sum to the A-B total, the factors act independently
and one table explains everything. If they do not, there is interaction, and
`--design full` (all 3^4 = 81 combinations, ~8x the compute) is warranted.

Prompts are built by importing the ORIGINAL generator's templates and
`build_experiment_prompt`, never by re-typing strings, so every cell is
byte-identical to how the corresponding prompt would have been generated in
`build_conflict_awareness_dataset.py`.  The endpoints A and B already exist in
`behavior_results.jsonl`, and the script checks that its own endpoint margins
reproduce those cached values.  A decomposition whose endpoints do not
reproduce is not trustworthy, and the run says so rather than reporting it.

Only `false_relevant` prompts under the `neutral` policy are used: that is the
cell the swing lives in.  `--include-policy-endpoints` additionally scores the
two endpoints under the context and parametric instructions, which measures the
instruction effect on the SAME facts in the SAME run -- so the headline
"a paraphrase is worth more than an instruction" becomes an internal
comparison rather than a number quoted across runs.

Usage
-----
python E6_template_decomposition.py --config config.yaml \\
    --behavior results/gemma4_12b_conflict/behavior_results.jsonl \\
    --out      results/gemma4_12b_conflict/analysis/decomposition \\
    --validate-only          # build prompts, check them, no GPU
