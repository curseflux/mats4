...
E6 showed that one sentence carries ~90% of the 31-logit deference swing on
elements (0.8% -> 63.6% context-following from swapping the claim alone).  But
the two sentences it contrasted differ in four ways at once:

  LOW  In a classroom periodic-table display, Cf is shown as the symbol
       representing Actinium.
  HIGH A textbook section about Actinium uses Cf as the element's chemical
       symbol throughout the chapter.

    SPEECH ACT   "is shown as" asserts a fact about the world;
                 "uses X as" describes a source ADOPTING a convention.
    SOURCE       a classroom wall display vs a textbook (authority).
    PERSISTENCE  nothing vs "throughout the chapter".
    LEXICON      the specific words "uses" / "shown", which may be all it is.

So E6 licenses "a sentence", not "a property".  This script crosses those four
factors in a controlled 2x2x2x2 and measures each main effect.

    ACT          stipulate  "{source} uses {A} as the chemical symbol for {S}."
                 assert     "{source} states that the chemical symbol for {S} is {A}."
    REALIZATION  a second lexical form of each act, so a result cannot be about
                 one verb: "treats {A} as ..." and "reports ... to be {A}".
    SOURCE       "A university chemistry textbook" vs "A classroom wall poster".
    PERSISTENCE  "" vs "consistently".

Everything else -- subject, false answer, filler, question, response constraint
-- is held fixed at the relation's E6 low-endpoint frame.  Both relations'
low-endpoint fillers are source-neutral ("The surrounding material describes",
"It goes on to mention"), which is what makes this design clean where E6's was
not: the capitals fillers that E6 had to swap carry definite descriptions ("The
guidebook", "The article") that corefer with the claim's source, so claim and
filler were never independent factors there.  Holding one neutral filler fixed
removes that coupling instead of trying to model it.

Anchors, scored in the same run:

    orig_high / orig_low   the two original E6 claim sentences. With the frame
                           held fixed these ARE the E6 `low_plus_claim` and
                           `endpoint_low` cells, so their prompts must match
                           byte-for-byte and their margins must reproduce.
    bare                   "The chemical symbol for {S} is {A}." -- an
                           unattributed assertion with no source at all, the
                           standard setup in the knowledge-conflict literature.

Specificity control: the two extreme act levels are also run with the claim
about a DIFFERENT entity (the fact's distractor). If a persuasive phrasing
raises the claim answer's probability even when the claim is not about the
question, the effect is salience/copying, not conflict resolution.

Cells: 16 factorial + 3 anchors + 2 controls = 21 per fact, both relations,
neutral policy only. About 5.5k prompts, ~1.5x the E6 run.

The output is deliberately four numbers per relation. A large experiment can
still be a small result; do not put the 21-cell table in the write-up.

Usage
-----
python E7_claim_phrasing.py --config config.yaml \\
    --behavior   results/gemma4_12b_conflict/behavior_results.jsonl \\
    --e6-results results/gemma4_12b_conflict/analysis/decomposition/decomposition_results.jsonl \\
    --out        results/gemma4_12b_conflict/analysis/phrasing \\
    --validate-only
"""
